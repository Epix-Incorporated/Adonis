...
return function(Vargs, GetEnv)
	...
	local script, getfenv, pcall, unpack, string, tostring, type, Enum =
		script, getfenv, pcall, unpack, string, tostring, type, Enum
	...
	local function RunAfterLoaded()
		...
		service.Player.CharacterRemoving:Connect(
			service.EventTask("Event: CharacterRemoving", Process.CharacterRemoving)
		)
		service.Player.CharacterAdded:Connect(
			service.Threads.NewEventTask("Event: CharacterAdded", Process.CharacterAdded)
		)
	...
	client.Process = {
		...
		LogService = function(Message, Type) end,
		...
		ErrorMessage = function(Message, Trace, Script)
			if
				...
				and Message ~= "nil"
				and Message ~= ""
				and (string.find(Message, ":: Adonis ::") or string.find(Message, script.Name) or Script == script)
			then
