...
return function(Vargs, GetEnv)
	...
	local getfenv, setmetatable, print, math, warn, error, pcall, select, os, time, unpack, string, Color3, tostring, table, type, Enum, delay =
		getfenv,
		setmetatable,
		print,
		math,
	...
	local function RunAfterLoaded()
		...
		task.delay(5, function()
			service.StartLoop("ClientCheck", 30, Remote.CheckClient, true)
		end)
	...
	client.Remote = {
		...
		GetFire = function(...)
			...
			if RemoteEvent and RemoteEvent.Function then
				...
				service.Queue("REMOTE_SEND", function()
					...
					task.delay(
						0,
						function() -- Wait for return in new thread; We don't want to hold the entire fire queue up while waiting for one thing to return since we just want to limit fire speed;
							returns = {
		...
		RawGet = function(...)
			...
			if RemoteEvent and RemoteEvent.Function then
				Remote.Sent = Remote.Sent + 1
				return RemoteEvent.Function:InvokeServer({
					Mode = "Get",
					Module = client.Module,
					Loader = client.Loader,
					Sent = Remote.Sent,
