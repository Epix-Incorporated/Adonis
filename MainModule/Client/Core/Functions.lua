...
return function(Vargs, GetEnv)
	...
	local game, getfenv, workspace, print, math, pcall, time, string, Color3, tostring, tonumber, Instance, BrickColor, table, type, Enum, UDim2, Vector3, CFrame =
		game,
		getfenv,
		workspace,
		print,
	...
	client.Functions = {
		...
		NewCape = function(options)
			...
			local decal = if decalId and decalId ~= 0
				then service.New("Decal", {
					Parent = p,
					Name = "Decal",
					Face = 2,
					...
					for i, v in Variables.Capes do
						...
						if parent and parent.Parent and torso and torso.Parent and part and part.Parent then
							if v.Enabled and Variables.CapesEnabled then
								...
								ang += math.min(torso.Velocity.Magnitude / 11, 0.8)
		...
		PlayAnimation = function(animId)
			...
			local animator = human and human:FindFirstChildOfClass("Animator")
				or human and human:WaitForChild("Animator", 9e9)
		...
		KeyBindListener = function(keybinds)
			...
			service.UserInputService.InputBegan:Connect(function(input)
				...
				if
					Variables.KeybindsEnabled
					and not textbox
					and key
					and Variables.KeyBinds[key]
					and not Variables.WaitingForBind
				then
		...
		Cape = function(material, color, decal, reflect)
			...
			if torso then
				...
				repeat
					...
					ang += math.min(torso.Velocity.Magnitude / 11, 0.5)
					...
					repeat
						task.wait()
					until motor1.CurrentAngle == motor1.DesiredAngle
