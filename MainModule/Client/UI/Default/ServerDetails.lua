...
return function(data, env)
	...
	do
		...
		local entries = {
			...
			{
				"Game Creator",
				service.MarketPlace:GetProductInfo(game.PlaceId).Creator.Name .. " (#" .. data.CreatorId .. ")",
			},
		...
		elseif serverType == "Private" then
			...
			table.insert(entries, 11, {
				...
				(service.Players:GetNameFromUserIdAsync(data.PrivateServerOwnerId) or "[Unknown Username]")
					.. " ("
					.. data.PrivateServerOwnerId
					.. ")",
			})
		end
		...
		spawn(function()
			while wait(0.5) do
				...
				if not displays["Server Speed"] or not displays["Server Age"] then
					break
				end
				displays["Server Speed"].Text = math.round(service.Workspace:GetRealPhysicsFPS())
				displays["Server Age"].Text = string.format(
					"%ds (%d min)",
					string.format(timeNow - data.ServerStartTime, "%.1f"),
					math.round((timeNow - data.ServerStartTime) / 60)
				)
			end
		end)
	end
	...
	do
		local function show()
			...
			if data.ServerInternetInfo then
				...
				table.insert(
					entries,
					{ "Error: Unable to retrieve server location info", "HTTP requests may be disabled for this game." }
				)
			end
	...
	do
		...
		function genPlayerList()
				...
				spawn(function()
					entry:Add("ImageLabel", {
						Image = service.Players:GetUserThumbnailAsync(
							service.Players[playerName].UserId,
			...
			search.PlaceholderText = "Players: "
				.. playerCount
				.. (data.Refreshables.Admins and " | Admins: " .. adminCount or "")
				.. " | Donors: "
				.. #data.Refreshables.Donors
		end
