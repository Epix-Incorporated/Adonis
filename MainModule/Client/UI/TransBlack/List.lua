...
return function(data, env)
	...
	function doSearch(tab, text)
		...
		for _, v in ipairs(tab) do
			if
				text == ""
				or (type(v) == "string" and string.find(string.lower(v), text))
				or (
					type(v) == "table"
					and (
						(v.Text and string.find(string.lower(tostring(v.Text)), text))
						or (v.Filter and string.find(string.lower(v.Filter), text))
					)
				)
			then
	...
	search = window:Add("TextBox", {
		...
		TextSize = 14,
		TextWrapped = true, --// It will look weird if we don't do this!
		TextXAlignment = Enum.TextXAlignment.Left,
	})
	pcall(function()
		search.Parent = window.Parent.Parent --// This is the easiest way i could get TransBlack style search box
	end) --// this SHOULD be drag if it isnt... Then stuff will hit the fan.
