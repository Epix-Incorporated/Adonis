...
return function(Vargs, GetEnv)
	...
	local Functions, Commands, Admin, Core, Logs, Remote, Process, Deps =
		server.Functions,
		server.Commands,
		server.Admin,
		server.Core,
	...
	Core.CrossServerCommands = {
		...
		NewRunCommand = function(_, plrData, comString)
			Process.Command(
				Functions.GetFakePlayer(plrData),
				comString,
				{ AdminLevel = plrData.AdminLevel, CrossServer = true }
			)
		end,
		...
		RemovePlayer = function(_, name, BanMessage, reason)
			--// probably should move this to userid
			local player = service.Players:FindFirstChild(name)
			if player then
				player:Kick(`{BanMessage} | Reason: {reason}`)
			end
		end,

		DataStoreUpdate = function(_, key, data)
			if key and data then
				Routine(Core.LoadData, key, data)
			end
		end,

		UpdateSetting = function(_, setting, newValue)
			if type(setting) == "string" then
				Settings[setting] = if newValue == nil
					then require(Deps.DefaultSettings).Settings[setting]
					else newValue
			end
		end,
		...
		Event = function(_, eventId, ...)
			service.Events[`CRSSRV:{eventId}`]:Fire(...)
		end,

		CrossServerVote = function(_, data)
			...
			for _, v in service.GetPlayers() do
				Routine(function()
					...
					if response and os.time() - start <= 120 then
						MsgService:PublishAsync(
							voteKey,
							{ PlrInfo = { Name = v.Name, UserId = v.UserId }, Response = response }
						)
					end
				end)
			end
	...
	Commands.CrossServer = {
		...
		Function = function(plr: Player, args: { string })
			if
				not Core.CrossServer("NewRunCommand", {
					UserId = plr.UserId,
					Name = plr.Name,
	...
	Commands.CrossServerVote = {
		...
		Function = function(plr: Player, args: { string })
			...
			if not question then
				error("You forgot to supply a question! (argument #2)")
			end
