...
do
	...
	-- Adonis Remote
	local adonisRemote = script.Parent.Parent:FindFirstChildOfClass("RemoteFunction")
	adonisRemote.OnClientInvoke = function(action)
		if action == "Disable" and enabled then
			enabled = false
			StopFreecam()
		end

		if action == "Enable" and not enabled then
			enabled = true
			StartFreecam()
		end
		...
		if action == "End" or action == "Stop" then
			adonisRemote.OnClientInvoke = nil
			...
			freecamToggleAction.Enabled = false -- disable toggling
			if StarterPlayer:FindFirstChild("FreecamInput") then -- remove the starterplayer folder
				StarterPlayer.FreecamInput:Destroy()
			end
			
			Debris:AddItem(adonisRemote, 0.5)
		end
	end
end
