...
----------------------------------------------
	...
	**HOW TO ADD ADMINISTRATORS:**
	...
	settings.AntiRootJointDeletion = true		-- (Also very important, this is unusual behavior.) Attempts to detect paranoid and kills the player if found
	settings.AntiHumanoidDeletion = true  -- (If so important, trial default = true?) (Very important) Prevents invalid humanoid deletion. Un-does the deletion and kills the player
	settings.AntiMultiTool = true -- (Worth trialling default = true Prevents multitooling and because of that many other exploits
	...
	settings.AntiBuildingTools = true	-- (Worth trialling default = true) (Client-Sided) Attempts to detect any HopperBin(s)/Building Tools added to the client
	settings.AntiLeak = false			-- (Client-Sided) Attempts to prevent place downloading/saving; Do not use if game saves
	settings.ProtectHats = true  				-- (Worth trialling) Prevents hats from being un-welded from their characters through unnormal means.
