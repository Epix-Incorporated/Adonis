...
--------------
	...
	--[[ Basic Luau Info
		...
		or an item they own I would do the following with the Admins table:
		...
		Admins = {"Bobjenkins123", "Bobjenkins123:1234567", 1234567, "Group:BobFans:7463213:234", "Item:1237465"}


		If I wanted to make it so rank 134 in group 1029934 and BobJenkins123 had mod admin I would do
		Moderators = {"Group:1029943:134", "BobJenkins123"}
		...
		--// Admins
			...
			Moderators = {"Sceleratis"; "BobJenkins:1237123"; 1237666; "Group:181:255"; "Item:1234567"}
		...
		--// Trello
			...
			MAKE SURE YOU SET DataStoreKey TO ANYTHING RANDOM.
																																																																									--]]
local settings = {
	...
	RecursiveTools = false;					 -- Whether tools that are included in sub-containers within settings.Storage will be available via the :give command (useful if your tools are organized into multiple folders)
	...
	**HOW TO ADD ADMINISTRATORS:**
	...
	Banned = {};		-- List of people banned from the game 		  Format: {"Username"; "Username:UserId"; UserId; "Group:GroupId:GroupRank"; "Group:GroupId"; "Item:ItemID"; "GamePass:GamePassID";}
	Muted = {};			-- List of people muted (cannot send chat messages)				 		  Format: {"Username"; "Username:UserId"; UserId; "Group:GroupId:GroupRank"; "Group:GroupId"; "Item:ItemID"; "GamePass:GamePassID";}
	Blacklist = {};		-- List of people banned from running commands 	  Format: {"Username"; "Username:UserId"; UserId; "Group:GroupId:GroupRank"; "Group:GroupId"; "Item:ItemID"; "GamePass:GamePassID";}
	Whitelist = {};		-- People who can join if whitelist enabled	  Format: {"Username"; "Username:UserId"; UserId; "Group:GroupId:GroupRank"; "Group:GroupId"; "Item:ItemID"; "GamePass:GamePassID";}
	...
	PlayerPrefix = "!";			-- The ! in !donate; Mainly used for commands that any player can run; Do not make it the same as settings.Prefix
...
--[[
	...
	Detection = true;			-- (Default: true) 	If true: enables built-in anti-exploit detections that do not have their own settings.
	CheckClients = true;		-- (Default: true) 	Checks clients every minute or two to make sure they are still active.

	ExploitNotifications = true;        -- (Default: true)	Notify all moderators and higher-ups when a player is kicked or crashed from the AntiExploit.
	CharacterCheckLogs = false;			-- (Default: false)	If the character checks appear in exploit logs and exploit notifications.
	AntiNoclip = false;					-- (Default: false)	Attempts to detect noclipping and kills the player if found.
	AntiRootJointDeletion = false;		-- (Default: false)	Attempts to detect paranoid and kills the player if found.
	AntiMultiTool = false; 				-- (Default: false)	Prevents multitool and because of that many other exploits.
	AntiGod = false; 					-- (Default: false)	If a player does not respawn when they should have they get respawned.
	-- AntiHumanoidDeletion and ProtectHats have been superseded Workspace.RejectCharacterDeletions.

	AntiSpeed = false; 				-- (Default: false)	(Client-Sided) Attempts to detect speed exploits.
...
local descs = {			--// Contains settings descriptions
	...
	CreatorPowers = [[ Gives me creator-level admin; This is strictly used for debugging; I can't debug without access to the script and specific owner commands ]];
...
return {Settings = settings, Descriptions = descs, Order = descs.order}
