			...
			if r and class and obj[index] and type(obj[index]) == "function" then
				...
				if RealMethods[class][index] ~= obj[index] then
					if ErrorHandler then
						ErrorHandler(
							"MethodError",
							`{debug.traceback()} || Cached method doesn't match found method: {tostring(index)}`,
							`Method: {index}`,
							index
						)
					end
...
return function(errorHandler, eventChecker, fenceSpecific, env)
	...
	local _G, game, getfenv, getmetatable, setmetatable, coroutine, rawequal, typeof, math, warn, error, pcall, select, rawset, ipairs, os, time, unpack, string, newproxy, tostring, tonumber, Instance, table, type, wait, Enum, delay, task =
		_G,
		game,
		getfenv,
		getmetatable,
	...
	Instance = {
		new = function(obj, parent)
			obj = oldInstNew(obj)
			if parent then
				obj.Parent = service.UnWrap(parent)
			end
			return service and client and service.Wrap(obj, true) or obj
		end,
	}
	local Events, Threads, Wrapper, Helpers =
		{
			...
			TaskScheduler = function(taskName, props)
				props = props or {}
				if not props.Temporary and TaskSchedulers[taskName] then
					return TaskSchedulers[taskName]
				end
			...
			GetEvent = function(name)
				if not HookedEvents[name] then
					...
					event:SetSpecial("Connect", function(i, func)
						local special = math.random()
						local event2 = Wrap(
							UnWrap(event.Event):Connect(function(con, ...)
								local packedResult = table.pack(...)
		...
		},
		{
			...
			NewTask = function(name, func, timeout)
				...
				newTask = {
					...
					Finished = {},
					Function = function(...)
						newTask.R_Status = "Running"
						newTask.Running = true
						local ret = { func(...) }
						newTask.R_Status = "Finished"
						newTask.Running = false
						newTask.Remove()
						return unpack(ret)
					end,
					Remove = function()
						newTask.R_Status = "Removed"
						newTask.Running = false
						for i, v in service.Threads.Tasks do
							if v == newTask then
								table.remove(service.Threads.Tasks, i)
							end
						end
					...
					Resume = function(...)
						newTask.R_Status = "Resumed"
						newTask.Running = true
						newTask.Changed:Fire("Resumed")
						local rets = { service.Threads.Resume(newTask.Thread, ...) }
						if not rets[1] then
							ErrorHandler("TaskError", rets[2])
							newTask.Changed:Fire("Errored", rets[2])
							newTask.Remove()
						end
						return unpack(rets)
					end,
			...
			Remove = function(thread)
				service.Threads.Stop(thread)
				for ind, th in service.Threads.Threads do
					if th == thread then
			...
			Wrap = function(object, fullWrap)
				...
				elseif
					(
						...
						or typeof(object) == "RBXScriptSignal"
						or typeof(object) == "RBXScriptConnection"
					) and not service.Wrapped(object)
				then
					local UnWrap = service.UnWrap
					...
					newMeta.__tostring = function()
						return custom.ToString or tostring(object)
					end
		...
		},
		{
			...
			Timer = function(t, func, check)
				local start = time()
				local event
				event = service.RunService.RenderStepped:Connect(function()
			...
			Filter = function(str, from, to)
				...
				for i = 1, #lines do
					local ran, newl = pcall(function()
						return service.TextService
							:FilterStringAsync(lines[i], from.UserId)
							:GetChatForUserAsync(to.UserId)
					end)
			...
			FormatTime = function(optTime, options)
				...
				if not formatString then
					formatString = options.WithWrittenDate and "LL HH:mm:ss"
						or (options.WithDate and "L HH:mm:ss" or "HH:mm:ss")
				end
			...
			FormatPlayer = function(plr, withUserId)
				...
				local str = if plr.DisplayName == plr.Name
					then `@{plr.Name}`
					else `{plr.DisplayName or "???"} ({plr.Name or "???"})`
				if withUserId then
			...
			FindClass = function(parent, class)
				for ind, child in parent:GetChildren() do
			...
			ReadOnly = function(tabl, excluded, killOnError, noChecks)
				...
				local settings, getMeta, get, pc = getfenv().settings, getmetatable, getfenv, pcall
				local unique = doChecks and getMeta(get())

				return service.NewProxy({
					__index = function(tab, ind)
						ind = (type(ind) ~= "table" and typeof(ind) ~= "userdata") and ind
							or "Potentially dangerous index"
						...
						if
							...
							and (
								setRan
								or (get ~= getfenv or getMeta ~= getmetatable or pc ~= pcall)
								or (not topEnv or type(topEnv) ~= "table" or getMeta(topEnv) ~= unique)
							)
						then
							ErrorHandler(
								"ReadError",
								"Tampering with Client [read rt0001]",
								`[${tostring(ind)} {tostring(topEnv)} {tostring(topEnv and getMeta(topEnv))}\n{tostring(debug.traceback())}`
							)
						elseif
							tabl[ind] ~= nil
							and type(tabl[ind]) == "table"
							and not (excluded and (excluded[ind] or excluded[tabl[ind]]))
						then
					...
					__newindex = function(tab, ind, new)
						...
						then
							ErrorHandler(
								"ReadError",
								"Tampering with Client [write wt0003]",
								`[${tostring(ind)} {tostring(topEnv)} {tostring(topEnv and getMeta(topEnv))}\n{tostring(debug.traceback())}`
							)
						elseif not (excluded and (excluded[ind] or excluded[tabl[ind]])) then
							if killOnError then
								ErrorHandler(
									"ReadError",
									"Tampering with Client [write wt0005]",
									`[{tostring(ind)} {tostring(topEnv)} {tostring(topEnv and getMeta(topEnv))}]\n{tostring(debug.traceback())}`
								)
							end
				...
				if type(object) == "userdata" and service.HasItem(object, "Parent") then
					if object.Parent == nil then
						...
						if not ran then
							if
								err
								and string.match(err, "^The Parent property of (.*) is locked, current parent: NULL,")
							then
								return true
							else
								return false
	...
	service = setmetatable({
		...
		Delete = function(obj, num)
			game:GetService("Debris"):AddItem(obj, (num or 0))
			pcall(obj.Destroy, obj)
		end,
		RbxEvent = function(signal, func)
			local event = signal:Connect(func)
			table.insert(RbxEvents, event)
			return event
		end,
		SelfEvent = function(signal, func)
			service.RbxEvent(signal, function(...)
				func(...)
			end)
		end,
		DelRbxEvent = function(signal)
		...
		SanitizeString = function(str)
			str = service.Trim(str)
			local new = ""
			for i = 1, #str do
				if string.sub(str, i, i) ~= "\n" and string.sub(str, i, i) ~= "\0" then
					new = `{new}{string.sub(str, i, i)}`
				end
			end
			return new
		end,
		...
		RawEqual = function(obj1, obj2)
			return service.UnWrap(obj1) == service.UnWrap(obj2)
		end,
		CheckProperty = function(obj, prop)
			return pcall(function()
				return obj[prop]
			end)
		end,
		NewWaiter = function()
			local event = service.New("BindableEvent")
			return { Wait = event.wait, Finish = event.Fire }
		end,
	}, {
		__index = function(tab, index)
	...
	for i, v in Wrapper do
		if type(v) == "function" then
			WrapService:SetSpecial(i, function(ignore, ...)
				return v(...)
			end)
		else
	...
	for i, v in Helpers do
		if type(v) == "function" then
			HelperService:SetSpecial(i, function(ignore, ...)
				return v(...)
			end)
		else
	...
	for i, v in Threads do
		if type(v) == "function" then
			ThreadService:SetSpecial(i, function(ignore, ...)
				return v(...)
			end)
		else
	...
	for i, v in Events do
		if type(v) == "function" then
			EventService:SetSpecial(i, function(ignore, ...)
				return v(...)
			end)
		else
	...
	do
		...
		service:SetSpecial("IsA", function(i, check)
			return check == name
		end)
	end
