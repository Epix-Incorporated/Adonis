	...
	local function RunAfterPlugins()
		...
		remoteParent.ChildRemoved:Connect(function(c)
			...
			if
				...
				and not Core.FixingEvent
				and (function()
					for _, v in Core.RemoteEvent do
						if c == v then
							return true
						end
					end
	...
	server.Core = {
		...
		DisconnectEvent = function()
			if Core.RemoteEvent and not Core.FixingEvent then
				...
				pcall(function()
					service.Delete(Core.RemoteEvent.Object)
				end)
				pcall(function()
					service.Delete(Core.RemoteEvent.Function)
				end)
			...
			if keys then
				...
				event = service.Events.ClientLoaded:Connect(function(plr)
					if p == plr and container.Parent == parentObj then
						...
						local leaveEvent
						leaveEvent = p.AncestryChanged:Connect(function() -- after/on remove, not on removing...
							task.wait()
		...
		ExecutePermission = function(scr, code, isLocal)
			...
			for _, val in Core.ExecuteScripts do
				if not isLocal or (isLocal and val.Type == "LocalScript") then
					if
						(service.UnWrap(val.Script) == fixscr or code == val.Code)
						and (not val.runLimit or (val.runLimit ~= nil and val.Executions <= val.runLimit))
					then
						val.Executions += 1
		...
		Bytecode = function(str: string)
			if Core.BytecodeCache[str] then
				return Core.BytecodeCache[str]
			end
			...
			if not Core.PlayerData[key] then
				...
				if Core.DataStore then
					...
					if type(data) == "table" then
						data.AdminNotes = if data.AdminNotes
							then Functions.DSKeyNormalize(data.AdminNotes, true)
							else {}
		...
		SavePlayerData = function(p, customData)
			...
			if Core.DataStore then
				if pData then
					...
					for _, blacklistedData in ipairs({
						"LastChat",
						"AdminRank",
						"AdminLevel",
						"LastLevelUpdate",
		...
		SaveAllPlayerData = function()
			...
			for key, pdata in Core.PlayerData do
				...
				if
					player
					and (not pdata.LastDataSave or os.time() - pdata.LastDataSave >= Core.DS_AllPlayerDataSaveInterval)
				then
		...
		RemoveData = function(key)
			...
			if DataStore then
				local ran2, err2 = Queue(`DataStoreWriteData{tostring(key)}`, function()
					local ran, ret =
						Core.DS_WriteLimiter("Write", DataStore.RemoveAsync, DataStore, Core.DataStoreEncode(key))
					if ran then
		...
		SetData = function(key: string, value: any?, repeatCount: number?)
			...
			if DataStore then
				if value == nil then
					...
					local ran2, err2 = Queue(`DataStoreWriteData{tostring(key)}`, function()
						local ran, ret = Core.DS_WriteLimiter(
							"Write",
							DataStore.SetAsync,
							DataStore,
							Core.DataStoreEncode(key),
							value
						)
						if ran then
		...
		DoSave = function(data: TableData)
			...
			elseif data.Type == "TableRemove" then
				...
				local CheckMatch = if type(data) == "table" and data.LaxCheck
					then Functions.LaxCheckMatch
					else Functions.CheckMatch
				Core.UpdateData(key, function(sets)
					...
					for i, v in sets do
						if type(i) ~= "number" then
							sets[i] = nil
						elseif
							(CheckMatch(tab, v.Table) or CheckMatch(originalTable, v.Table))
							and CheckMatch(v.Value, val)
						then
			...
			if key == "TableUpdate" then
				...
				if type(displayName) == "string" then
					...
					if type(indList) == "table" and indList[1] == "Settings" and indList[2] == "Ranks" then
						...
						if
							type(indList[3]) == "string"
							and (indList[3]:match("Trello") or indList[3]:match("WebPanel"))
						then
							return
						end
					end
		...
		StartAPI = function()
			...
			local API = {
				Access = MetaFunc(function(...)
					...
					if
						Settings.G_Access
						and key == Settings.G_Access_Key
						and targ
						and Settings.Allowed_API_Calls[ind] == true
					then
							...
							return service.NewProxy({
								__index = function(tab, inde)
									if targ[inde] ~= nil and API_Special[inde] == nil or API_Special[inde] == true then
										...
										if
											targ[inde] ~= nil
											and type(targ[inde]) == "table"
											and Settings.G_Access_Perms == "Read"
										then
					...
					StartLoop("APICheck", 1, function()
						if rawget(_G, "Adonis") ~= AdonisGTable then
							if table.isfrozen and not table.isfrozen(_G) or not table.isfrozen then
								...
								warn(
									"⚠️ ADONIS CRITICAL WARNING! MALICIOUS CODE IS TRYING TO CHANGE THE ADONIS _G API AND IT CAN'T BE SET BACK! PLEASE SHUTDOWN THE SERVER AND REMOVE THE MALICIOUS CODE IF POSSIBLE!"
								)
							end
						end
					end, true)
