	...
	return {
		...
		SetCoreGuiEnabled = {
			...
			Function = function(plr: Player, args: { string })
				...
				local enable = if args[3]:lower() == "on" or args[3]:lower() == "true"
					then true
		...
		Alert = {
			...
			Function = function(plr: Player, args: { string })
				for _, v in service.GetPlayers(plr, string.lower(args[1])) do
					Remote.MakeGui(v, "Alert", {
						Message = args[2] and service.Filter(args[2], plr, v) or "Wake up; Your attention is required",
					})
				end
			end,
		},
		...
		Note = {
			...
			Function = function(plr: Player, args: { string })
				...
				for _, v in service.GetPlayers(plr, args[1]) do
					...
					if not PlayerData.AdminNotes then
								...
								local filteredMessage = select(
									...
									xpcall(function()
										return service.TextService
											:FilterStringAsync(message, v.UserId, Enum.TextFilterContext.PrivateChat)
										...
										Remote.Send(
											...
											"Function",
											"ChatMessage",
											"A message filtering error occurred.",
											Color3.new(1, 64 / 255, 77 / 255)
										)
										return
									end)
								)
