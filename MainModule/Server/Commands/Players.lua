	...
	return {
					...
					TitleButtons = {
						{
							...
							OnClick = Core.Bytecode(
								"client.Remote.Send('ProcessCommand','" .. Settings.PlayerPrefix .. "usage')"
							),
						},
					},
				})
		...
		CommandInfo = {
			...
			Function = function(plr: Player, args: { string })
				...
				local cmdAttribs = {}
				for _, key in
					{ "Disabled", "Fun", "Hidden", "NoStudio", "NonChattable", "CrossServerDenied", "AllowDonors" }
				do
					if cmd[key] then
				...
				Remote.MakeGui(plr, "List", {
					...
					Table = {
						...
						{
							Text = "<b>Commands:</b> " .. SanitizeXML(table.concat(cmd.Commands, ", ")),
							Desc = "Valid default aliases for the command",
						},
						{
							Text = "<b>Arguments:</b> " .. (if cmdArgs == "" then "-" else SanitizeXML(cmdArgs)),
							Desc = "Parameters taken by the command",
						},
						{
							Text = "<b>Admin Level:</b> " .. Admin.FormatCommandAdminLevel(cmd),
							Desc = "Rank required to run the command",
						},
						{ Text = "<b>Description:</b> " .. SanitizeXML(cmd.Description), Desc = "Command description" },
						{
							Text = "<b>Attributes:</b> "
								.. (if #cmdAttribs == 0 then "-" else table.concat(cmdAttribs, "; ")),
							Desc = "Extra data about the command",
						},
						{
							Text = "<b>Index:</b> " .. SanitizeXML(tostring(ind)),
							Desc = "The internal command index/identifier",
						},
					},
					RichText = true,
					Size = { 400, 225 },
				...
				if Settings.HelpSystem == true then
					...
					elseif pending and pending.Pending then
						...
						service.TrackTask("Thread: " .. tostring(plr) .. " Help Request Handler", function()
							...
							repeat
								task.wait(0.5)
							until os.time() - w > 30 or answered
							...
							if not answered then
								Functions.Message(
									"Help System",
									"Sorry but no one is available to help you right now",
									{ plr }
								)
							end
						end)
		...
		Rejoin = {
			...
			AdminLevel = "Players",
			Function = function(plr: Player, args: { string })
				service.TeleportService:TeleportAsync(
					game.PlaceId,
					{ plr },
					service.New("TeleportOptions", {
		...
		Join = {
			...
			Function = function(plr: Player, args: { string })
				...
				if UserId then
					local success, found, _, placeId, jobId =
						pcall(service.TeleportService.GetPlayerPlaceInstanceAsync, service.TeleportService, UserId)
					if success then
						if found and placeId and jobId then
							...
							Functions.Hint(
								service.Players:GetNameFromUserIdAsync(UserId) .. " was not found playing this game",
								{ plr }
							)
						end
					else
					...
					for _, v in plr:GetFriendsOnline() do
						if v.VisitorId == UserId then
							if v.IsOnline and v.PlaceId and v.GameId then
								...
								Functions.Hint(
									string.format("Joining %s (%s)...", v.UserName, v.LastLocation or "unknown game"),
									{ plr }
								)
							else
		...
		Usage = {
			...
			Function = function(plr: Player, args: { string })
				local usage = {
					...
					"",
					"Put <i>/e</i> in front to silence commands in chat (<i>/e "
						.. Settings.Prefix
						.. "kill scel</i>) or enable chat command hiding in client settings",
					"Player commands can be used by anyone, these commands have <i>"
					...
					"Usage example: <i>"
						.. Settings.Prefix
						.. "kill "
						.. Settings.SpecialPrefix
						.. "all</i> (where <i>"
						.. Settings.SpecialPrefix
						.. "all</i> is the selector)",
					...
					"<i>"
						.. Settings.SpecialPrefix
						.. "random</i> - A random person in the server excluding those removed with -SELECTION",
					"<i>@USERNAME</i> - Targets a specific player with that exact username Ex: <i>"
						.. Settings.Prefix
						.. "god @Sceleratis </i> would give a player with the username 'Sceleratis' god powers",
					"<i>#NUM</i> - NUM random players in the server <i>"
						.. Settings.Prefix
						.. "ff #5</i> will ff 5 random players excluding those removed with -SELECTION.",
					...
					"<i>-SELECTION</i> - Inverts the selection, ie. will remove SELECTION from list of players to run command on. "
						.. Settings.Prefix
						.. "kill all,-%TEAM will kill everyone except players on TEAM",
					"<i>+SELECTION</i> - Readds the selection, ie. will readd SELECTION from list of players to run command on. "
						.. Settings.Prefix
						.. "kill all,-%TEAM,+Lethalitics will kill everyone except players on TEAM but also Lethalitics",
					"<i>radius-NUM</i> -- Anyone within a NUM-stud radius of you. "
						.. Settings.Prefix
						.. "ff radius-5 will ff anyone within a 5-stud radius of you.",
					"",
					"<b>――――― Repetition ―――――</b>",
					"Multiple player selections - <i>"
						.. Settings.Prefix
						.. "kill me,noob1,noob2,"
						.. Settings.SpecialPrefix
						.. "random,%raiders,$123456,"
						.. Settings.SpecialPrefix
						.. "nonadmins,-scel</i>",
					"Multiple Commands at a time - <i>"
						.. Settings.Prefix
						.. "ff me "
						.. Settings.BatchKey
						.. " "
						...
						.. Settings.Prefix
						.. "rocket jim</i>",
					"You can add a delay if you want; <i>"
						.. Settings.Prefix
						.. "ff me "
						.. Settings.BatchKey
						.. " !wait 10 "
						...
						.. " "
						.. Settings.Prefix
						.. "m hi we waited 10 seconds</i>",
					"<i>"
						.. Settings.Prefix
						.. "repeat 10(how many times to run the cmd) 1(how long in between runs) "
						.. Settings.Prefix
						.. "respawn jim</i>",
					"",
					"<b>――――― Reference Info ―――――</b>",
					...
					"<i>"
						.. Settings.Prefix
						.. "cmdinfo &lt;command&gt;</i> for detailed info about a specific command",
					...
					"<i>"
						.. Settings.Prefix
						.. "insertlist</i> for a list of insertable assets using "
						.. Settings.Prefix
						.. "insert",
				}
				Remote.MakeGui(plr, "List", {
		...
		Theme = {
			...
			Function = function(plr: Player, args: { string })
				...
				Remote.LoadCode(
					plr,
					"client.Variables.CustomTheme = " .. if args[1] then "[[" .. args[1] .. "]]" else "nil"
				)
			end,
		},
		...
		ViewProfile = {
			...
			Function = function(plr: Player, args: { string })
				...
				if #players > 2 then
					Functions.Hint(
						string.format("Loading profile data for %d players... [This may take a while]", #players),
						{ plr }
					)
				end
				...
				for _, v: Player in players do
					task.defer(function()
						...
						local policyInfo = elevated
							and select(
								2,
								xpcall(service.PolicyService.GetPolicyInfoForPlayerAsync, function()
									return "[Error]"
								end, service.PolicyService, v)
							)
						local hasSafeChat = if policyInfo
							then type(policyInfo) == "string" and policyInfo or table.find(
		...
		ServerDetails = {
			...
			Function = function(plr: Player, args: { string })
				...
				local serverInfo = select(
					2,
					xpcall(function()
						local res =
							service.HttpService:JSONDecode(service.HttpService:GetAsync("http://ip-api.com/json"))
						return {
							country = res.country,
							city = res.city,
							region = res.region,
		...
		GetGroupInfo = {
			...
			Function = function(plr: Player, args: { string })
				...
				local groupInfo = assert(
					select(
						2,
						xpcall(service.GroupService.GetGroupInfoAsync, function()
							return nil
						end, service.GroupService, groupId)
					),
					"Error fetching data"
				)
				local tab = {
					...
					"ID: " .. groupInfo.Id,
					if groupInfo.Owner
						then string.format("Owner: %s [%d]", groupInfo.Owner.Name or "???", groupInfo.Owner.Id)
						else "[No Owner]",
					{
						Text = string.format(
							"――― Roles (%d): ―――――――――――――――――",
