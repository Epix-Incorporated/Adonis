return function(Vargs, env)
	...
	local Functions, Commands, Admin, Anti, Core, HTTP, Logs, Remote, Process, Variables, Deps = server.Functions, server.Commands, server.Admin, server.Anti, server.Core, server.HTTP, server.Logs, server.Remote, server.Process, server.Variables, server.Deps
	...
	return {
								...
								if check() then
									...
									for i, v in next, p.Character:GetChildren() do
										if v:IsA("BasePart") then
											...
											v.Transparency = 1
											pcall(function()
												v:FindFirstChildOfClass("Decale"):Destroy()
											end)
		...
		Puke = {
			...
			Function = function(plr, args)
				for i, v in pairs(service.GetPlayers(plr, args[1])) do
					cPcall(function()
						if (not v:IsA("Player")) or not v or not v.Character or (not v.Character:FindFirstChild("Head")) or v.Character:FindFirstChild("Epix Puke") then
							return
						end
						local run = true
		...
		Cut = {
			...
			Function = function(plr, args)
				for i, v in pairs(service.GetPlayers(plr, args[1])) do
					cPcall(function()
						if (not v:IsA("Player")) or not v or not v.Character or (not v.Character:FindFirstChild("Head")) or v.Character:FindFirstChild("Epix Bleed") then
							return
						end
						local run = true
						local k = service.New("StringValue", v.Character)
				...
				for i, plr in pairs(service.GetPlayers(p, args[1])) do
					...
					if plr and plr.Character and plr.Character:FindFirstChild("HumanoidRootPart") then
						...
						if human.RigType == Enum.HumanoidRigType.R6 then
							...
							local weld = service.New("Weld", { Parent = st, Part0 = torso, Part1 = st, C1 = CFrame.new(0, 0.5, 0) })
				...
				for _, v in pairs(service.GetPlayers(plr, args[1])) do
					if v.Character and v.Character:FindFirstChild("HumanoidRootPart") then
						...
						if humanoid then
							...
							if rarm then
								rarm.C0 = isR15 and CFrame.new(-1, -1.5, -0.5) or (CFrame.new(0, -1.5, -0.5) * CFrame.Angles(0, math.rad(90), 0))
							end

							local larm = isR15 and joints["LeftShoulder"] or joints["Left Shoulder"]
							if larm then
								larm.C0 = isR15 and CFrame.new(1, -1.5, -0.5) or (CFrame.new(0, -1.5, -0.5) * CFrame.Angles(0, math.rad(-90), 0))
							end

							local rleg = isR15 and joints["RightHip"] or joints["Right Hip"]
							if rleg then
								rleg.C0 = isR15 and (CFrame.new(-0.5, -0.5, 0.5) * CFrame.Angles(0, math.rad(180), 0)) or (CFrame.new(0, -1, 0.5) * CFrame.Angles(0, math.rad(90), 0))
							end

							local lleg = isR15 and joints["LeftHip"] or joints["Left Hip"]
							if lleg then
								lleg.C0 = isR15 and (CFrame.new(0.5, -0.5, 0.5) * CFrame.Angles(0, math.rad(180), 0)) or (CFrame.new(0, -1, 0.5) * CFrame.Angles(0, math.rad(-90), 0))
							end
				...
				for i, v in next, service.GetPlayers(plr, args[1]) do
					...
					elseif human and human.RigType == Enum.HumanoidRigType.R6 then
						...
						for i, Accessory in pairs(char:GetChildren()) do
							if Accessory:IsA("Accessory") == false then
								continue
							end
