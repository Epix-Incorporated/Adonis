return function(Vargs, env)
	...
	return {
			...
			Function = function(plr: Player, args: { string }, data: {})
				...
				for i, p in players do
					if not Admin.CheckAuthority(plr, p, "timeout") then
						...
						Functions.Hint(
							"Unable to send "
								.. service.FormatPlayer(p)
								.. " to The Maze (insufficient permission level)",
							{ plr }
						)
					end
				end
		...
		HatPets = {
			...
			Description = "Gives the target player(s) hat pets, controlled using the "
		...
		Pets = {
			...
			Function = function(plr: Player, args: { string })
				...
				if hats then
					...
					Functions.Hint(
						"You don't have any hat pets! If you are an admin use the "
							.. Settings.Prefix
							.. "hatpets command to get some",
						{ plr }
					)
				end
			end,
		...
		Resize = {
			...
			Function = function(plr: Player, args: { string })
				...
				elseif tonumber(args[2]) and tonumber(args[2]) > sizeLimit then
					Functions.Hint(
						"Size changed to the maximum "
							.. tostring(num)
							.. " [Argument #2 (size multiplier) went over the size limit]",
						{ plr }
					)
				end
				...
				for _, v in service.GetPlayers(plr, args[1]) do
					...
					if not human then
						Functions.Hint(
							"Cannot resize "
								...
								.. "'s character: humanoid and/or character doesn't exist!",
							{ plr }
						)
						continue
					end
					...
					elseif Variables.SizedCharacters[char] and Variables.SizedCharacters[char] * num < sizeLimit then
						...
						Functions.Hint(
							string.format(
								"Cannot resize %s's character by %f%%: size limit exceeded.",
								service.FormatPlayer(v),
								num * 100
							),
							{ plr }
