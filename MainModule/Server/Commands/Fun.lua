	...
	return {
		...
		Gerald = {
			...
			Function = function(plr: Player, args: { string })
				...
				warn(
					"Requiring Assets Module by ID; Expand for module URL > ",
					{ URL = "https://www.roblox.com/library/7679952474/Adonis-Assets-Module" }
				)
				...
				for _, v: Player in service.GetPlayers(plr, args[1]) do
					task.defer(function()
						...
						if head then
							...
							service
								.New("Sound", { Parent = head, Volume = 3, SoundId = "rbxassetid://606862847" })
								:Play()
							local face = head:FindFirstChild("face")
							if face then
								face.Texture = "rbxassetid://412416747"
							end
							...
							service
								.New("Sound", {
									Parent = char:FindFirstChild("HumanoidRootPart")
		...
		Brazil = {
			...
			Function = function(plr, args)
				for _, v in service.GetPlayers(plr, args[1]) do
					...
					vel.Velocity = CFrame.new(
						root.Position - Vector3.new(0, 1, 0),
						root.CFrame.LookVector * 5 + root.Position
					).LookVector * 1500
					...
					smoke.Size = NumberSequence.new({
						NumberSequenceKeypoint.new(0, 1.25, 1.25),
						NumberSequenceKeypoint.new(1, 1.25, 1.25),
					})
		...
		CharGear = {
			...
			Function = function(plr: Player, args: { string })
				...
				local cfr = assert(
					plrChar:FindFirstChild("RightHand") or plrChar:FindFirstChild("Right Arm"),
					"You don't have a right hand/arm"
				).CFrame
		...
		Boombox = {
			...
			AdminLevel = "Moderators",
			Function = function(plr: Player, args: { string })
				local gear = service.Insert(212641536)
				...
				infect = function(v)
					...
					if char and char:FindFirstChild("HumanoidRootPart") and not char:FindFirstChild("Infected") then
						...
						for _, prt in char:GetChildren() do
							if
								prt:IsA("BasePart")
								and prt.Name ~= "HumanoidRootPart"
								and (prt.Name ~= "Head" or not prt.Parent:FindFirstChild("NameTag", true))
							then
								prt.Transparency = 0
								prt.Reflectance = 0
								...
								tconn = prt.Touched:Connect(function(hit)
									if
										hit
										and hit.Parent
										and service.Players:FindFirstChild(hit.Parent.Name)
										and cl.Parent == char
									then
										infect(hit.Parent)
					...
					if v.Character then
						for _, p in v.Character:GetChildren() do
							if
								p:IsA("Shirt")
								or p:IsA("Pants")
								or p:IsA("CharacterMesh")
								or p:IsA("Accoutrement")
								or p:IsA("BodyColors")
							then
								p:Destroy()
							end
						...
						if check() then
							...
							if check() then
								...
								if p and p.Parent == service.Players then
									service.TeleportService:Teleport(6806826116, p, nil, bg)
								end
								task.wait(0.5)
								pcall(function()
									gui:Destroy()
		...
		ScreenImage = {
			...
			Function = function(plr: Player, args: { string })
				...
				if not img then
					error(`{args[2]} is not a valid ID`)
				end
				for _, v in service.GetPlayers(plr, args[1]) do
					Remote.MakeGui(v, "Effect", {
						Mode = "ScreenImage",
			...
			Function = function(plr: Player, args: { string }, data: {})
				...
				for i, p in players do
					if not Admin.CheckAuthority(plr, p, "timeout") then
						table.remove(players, i)
						Functions.Hint(
							`Unable to send {service.FormatPlayer(p)} to The Maze (insufficient permission level)`,
							{ plr }
						)
					end
				end
					...
					service.TrackTask("Thread: Clowns", function()
						...
						for i = 1, 400 do
							if not check() then
								...
								torso.CFrame = primary.CFrame
									* (CFrame.new(0, 2.3, 0) * CFrame.Angles(0, math.rad(90), 0))
								task.wait(0.1 / (i * 5))
			...
			Function = function(plr, args)
				...
				service.StartLoop("ChickenSpam", 5, function()
					...
					for i, v in hats do
						...
						nhat.CFrame = v.CFrame
							* CFrame.new(math.random(-100, 100), math.random(-100, 100), math.random(-100, 100))
							* CFrame.Angles(math.random(-360, 360), math.random(-360, 360), math.random(-360, 360))
					end
					hats = tempHats
				end)
			...
			Function = function(plr: Player, args: { string })
				...
				local function fire(part)
					...
					elseif
						...
						and (not part.Locked or (part.Parent:IsA("Model") and service.Players:GetPlayerFromCharacter(
							part.Parent
						)))
						and part.Name ~= "BasePlate"
						and part.Name ~= "Baseplate"
						and not partsHit[part]
					then
						partsHit[part] = true
					...
					cPcall(function()
						if v.Character and v.Character:FindFirstChild("HumanoidRootPart") then
							...
							if knownchar.Parent then
								service.New("Explosion", workspace.Terrain).Position =
									knownchar.HumanoidRootPart.Position
				...
				for i, v in service.GetPlayers(plr, args[1]) do
					cPcall(function()
						...
						repeat
							...
							task.wait()
						until not hum or hum == nil or hum.Parent == nil
					end)
				end
			end,
		},
				...
				for i, v in service.GetPlayers(plr, args[1]) do
					cPcall(function()
						if
							...
							or v.Character:FindFirstChild("Epix Puke")
						then
							return
						end
						local run = true
						...
						Routine(function()
							repeat
								...
								p.Velocity = v.Character.Head.CFrame.lookVector * 20
								...
								p.Touched:Connect(function(o)
									if
										o
										and p
										and (not service.Players:FindFirstChild(o.Parent.Name))
										and o.Name ~= "Puke Peice"
										and o.Name ~= "Blood Peice"
										and o.Name ~= "Blood Plate"
										and o.Name ~= "Puke Plate"
										and (o.Parent:IsA("Workspace") or o.Parent:IsA("Model"))
										and (o.Parent ~= p.Parent)
										and o:IsA("Part")
										and (o.Parent.Name ~= v.Character.Name)
										and (not o.Parent:IsA("Accessory"))
										and (not o.Parent:IsA("Tool"))
									then
				...
				for _, v in service.GetPlayers(plr, args[1]) do
					cPcall(function()
						if
							...
							or not v.Character
							or not v.Character:FindFirstChild("Head")
							or v.Character:FindFirstChild("ADONIS_BLEED")
						then
							return
						end
						local run = true
						...
						Routine(function()
							repeat
								...
								p.Velocity = v.Character.Head.CFrame.lookVector * 1
				...
				for i, v in service.GetPlayers(plr, args[1]) do
					Routine(function()
						...
						if
							...
							and head
							and hum
							and not v.Character:FindFirstChild("Adonis_Poisoned")
						then
			...
			Function = function(plr: Player, args: { string })
				...
				if hats then
					for i, v in service.GetPlayers(plr, args[2]) do
						if
							v.Character:FindFirstChild("HumanoidRootPart") and v.Character.HumanoidRootPart:IsA("Part")
						then
					...
					Functions.Hint(
						`You don't have any hat pets! If you are an admin use the {Settings.Prefix}hatpets command to get some`,
						{ plr }
					)
				end
			end,
				...
				for i, v in service.GetPlayers(plr, args[1]) do
					if v.Character:FindFirstChild("HumanoidRootPart") then
						v.Character.HumanoidRootPart.CFrame = v.Character.HumanoidRootPart.CFrame
							+ Vector3.new(0, tonumber(args[2]), 0)
					end
				end
			end,
		},
				...
				for kay, player in service.GetPlayers(plr, args[1]) do
					Routine(function()
						...
						if torso then
							...
							for i = 1, 10 do
								torso.CFrame = foot
									* CFrame.fromEulerAnglesXYZ(-(math.pi / 2) * i / 10, 0, 0)
									* CFrame.new(0, 3, 0)
								task.wait(0.1)
							end
							for i = 1, 5, 0.2 do
				...
				for _, v in service.GetPlayers(plr, args[1]) do
					if v.Character and v.Character:FindFirstChild("HumanoidRootPart") then
						for _, q in v.Character.HumanoidRootPart:GetChildren() do
							if
								q.Name == "SPINNER"
								or q.Name == "ADONIS_SPIN_GYRO"
								or q.Name == "ADONIS_SPIN_GYRO_ATTACHMENT"
							then
								q:Destroy()
							end
						end
					...
					if plr and plr.Character and plr.Character:FindFirstChild("HumanoidRootPart") then
						...
						if not human then
							Remote.MakeGui(p, "Output", {
								Title = "Output",
								Message = `{plr.Name} doesn't have a Humanoid [Transformation Error]`,
							})
							return
						end
						...
						if human.RigType == Enum.HumanoidRigType.R6 then
							...
							torso["Right Shoulder"].C0 = CFrame.new(0.5, -1.5, -1.5) * ca1
							torso["Left Shoulder"].C0 = CFrame.new(-0.5, -1.5, -1.5) * ca2
						...
						elseif human.RigType == Enum.HumanoidRigType.R15 then
							Remote.MakeGui(p, "Output", {
								Title = "Output",
								Message = `Cannot support R15 for {plr.Name} [Dog Transformation Error]`,
							})
						end
					end
				end
				...
				for _, v in Functions.GetPlayers(plr, args[1]) do
					...
					for _, p in char:GetChildren() do
						if p:IsA("BasePart") then
							...
							local attachment0 = p:FindFirstChild("ADONIS_TRAIL_ATTACHMENT0")
								or service.New("Attachment", {
									Parent = p,
									Name = "ADONIS_TRAIL_ATTACHMENT0",
								})
							local attachment1 = p:FindFirstChild("ADONIS_TRAIL_ATTACHMENT1")
								or service.New("Attachment", {
					...
					elseif human and human.RigType == Enum.HumanoidRigType.R6 then
						...
						local function size(part)
							for _, v in part:GetChildren() do
								if
									(v:IsA("Weld") or v:IsA("Motor") or v:IsA("Motor6D"))
									and v.Part1
									and v.Part1:IsA("Part")
								then
									local p1 = v.Part1
				...
				for _, v in service.GetPlayers(plr, args[1]) do
					if v.Character and v.Character:FindFirstChild("HumanoidRootPart") then
						...
						if humanoid then
							...
							if joints["Neck"] then
								joints["Neck"].C0 = isR15 and CFrame.new(0, 1, 0)
									or (CFrame.new(0, 1, 0) * CFrame.Angles(math.rad(90), math.rad(180), 0))
							end

							local rarm = isR15 and joints["RightShoulder"] or joints["Right Shoulder"]
							if rarm then
								rarm.C0 = isR15 and CFrame.new(-1, -1.5, -0.5)
									or (CFrame.new(0, -1.5, -0.5) * CFrame.Angles(0, math.rad(90), 0))
							end

							local larm = isR15 and joints["LeftShoulder"] or joints["Left Shoulder"]
							if larm then
								larm.C0 = isR15 and CFrame.new(1, -1.5, -0.5)
									or (CFrame.new(0, -1.5, -0.5) * CFrame.Angles(0, math.rad(-90), 0))
							end

							local rleg = isR15 and joints["RightHip"] or joints["Right Hip"]
							if rleg then
								rleg.C0 = isR15 and (CFrame.new(-0.5, -0.5, 0.5) * CFrame.Angles(0, math.rad(180), 0))
									or (CFrame.new(0, -1, 0.5) * CFrame.Angles(0, math.rad(90), 0))
							end

							local lleg = isR15 and joints["LeftHip"] or joints["Left Hip"]
							if lleg then
								lleg.C0 = isR15 and (CFrame.new(0.5, -0.5, 0.5) * CFrame.Angles(0, math.rad(180), 0))
									or (CFrame.new(0, -1, 0.5) * CFrame.Angles(0, math.rad(-90), 0))
							end
					...
					if v.Character then
						...
						if human then
							if human.RigType == Enum.HumanoidRigType.R6 then
								...
								v.Character.Torso.Neck.C0 = CFrame.new(0, 1.3, 0)
					...
					if v.Character then
						...
						if human then
							if human.RigType == Enum.HumanoidRigType.R6 then
								...
								v.Character.Torso.Neck.C0 = CFrame.new(0, 0.8, 0)
									* CFrame.Angles(math.rad(90), math.rad(180), 0)
							else
								local scale = human and human:FindFirstChild("HeadScale")
								if scale then
		...
		Resize = {
			...
			Function = function(plr: Player, args: { string })
				...
				if not args[2] or not tonumber(args[2]) then
					num = 1
					Functions.Hint(
						"Size changed to 1 [Argument #2 (size multiplier) wasn't supplied correctly.]",
						{ plr }
					)
				elseif tonumber(args[2]) and tonumber(args[2]) > sizeLimit then
					Functions.Hint(
						`Size changed to the maximum {tostring(num)} [Argument #2 (size multiplier) went over the size limit]`,
						{ plr }
					)
				end
				...
				for _, v in service.GetPlayers(plr, args[1]) do
					...
					if not human then
						Functions.Hint(
							`Cannot resize {service.FormatPlayer(v)}'s character: humanoid and/or character doesn't exist!`,
							{ plr }
						)
						continue
					end
					...
					elseif Variables.SizedCharacters[char] and Variables.SizedCharacters[char] * num < sizeLimit then
						...
						Functions.Hint(
							string.format(
								`Cannot resize {service.FormatPlayer(v)}'s character by %f%%: size limit exceeded.`,
								num * 100
							),
							{ plr }
						)
					...
					elseif human and human.RigType == Enum.HumanoidRigType.R6 then
						...
						for _, v in char:GetDescendants() do
							...
							elseif v:IsA("Accessory") and v:FindFirstChild("Handle") then
								...
								handle.AccessoryWeld.C0 = CFrame.new((handle.AccessoryWeld.C0.Position * num))
									* (handle.AccessoryWeld.C0 - handle.AccessoryWeld.C0.Position)
								handle.AccessoryWeld.C1 = CFrame.new((handle.AccessoryWeld.C1.Position * num))
									* (handle.AccessoryWeld.C1 - handle.AccessoryWeld.C1.Position)
					...
					if v.Character then
						...
						if animateScript then
							...
							if not found then
								assert(
									nil,
									"Instance 'StringValue' named 'walk' was not found. Please note, this command is designed for the default animation system."
								)
							end
						else
					...
					if v.Character then
						...
						if animateScript then
							...
							if not found then
								assert(
									nil,
									"Instance 'StringValue' named 'run' was not found. Please note, this command is designed for the default animation system."
								)
							end
						else
							assert(nil, "Target player does not have the 'Animate' LocalScript")
					...
					if v.Character then
						...
						if animateScript then
							...
							if not found then
								assert(
									nil,
									"Instance 'StringValue' named 'jump' was not found. Please note, this command is designed for the default animation system."
								)
							end
						else
							assert(nil, "Target player does not have the 'Animate' LocalScript")
					...
					if v.Character then
						...
						if animateScript then
							...
							if not found then
								assert(
									nil,
									"Instance 'StringValue' named 'fall' was not found. Please note, this command is designed for the default animation system."
								)
							end
						else
							assert(nil, "Target player does not have the 'Animate' LocalScript")
		...
		LoadSky = {
			...
			Args = {
				...
				"up",
				"down",
				"celestialBodies? (default: true)",
				"starCount (default: 3000)",
			},
		...
		Explode = {
			...
			Args = {
				"player",
				"radius (default: 20 studs)",
				"blast pressure (default: 500,000)",
		...
		CreateSoundPart = {
			...
			Args = {
				"soundid",
				"soundrange (default: 10) (max: 100)",
				"pitch (default: 1)",
				"noloop (default: false)",
				"volume (default: 1)",
				"clicktotoggle (default: false)",
				"share type (default: everyone)",
			},
			...
			Function = function(plr: Player, args: { string })
				...
				local soundid = (args[1] and tonumber(args[1]))
					or select(1, function()
						if args[1] then
							local nam = args[1]
					...
					or error("SoundId wasn't provided or wasn't a valid number")
				...
				local Success, Return =
					pcall(service.MarketplaceService.GetProductInfo, service.MarketplaceService, soundid)
				...
				if Success and Return then
					...
					if toggable then
						...
						clickd.MouseClick:Connect(function(clicker)
							if sharetype == "self" and clicker.UserId ~= ownerid then
								return
							end
