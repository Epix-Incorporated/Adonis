return function(Vargs, env)
	...
	return {
		...
		TimeHint = {
			...
			AdminLevel = "Moderators";
			Function = function(plr: Player, args: {string})
				Functions.Hint(service.BroadcastFilter(assert(tonumber(args[1]), "Invalid time amount (must be a number)"), plr), service.GetPlayers(), tonumber(assert(args[2], "Missing message")), service.FormatPlayer(plr), `rbxthumb://type=AvatarHeadShot&id={plr.UserId}&w=48&h=48`)
			end
		};
