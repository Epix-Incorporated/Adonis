	...
	return {
		Kick = {
			...
			Function = function(plr: Player, args: { string }, data: {})
				for _, v in
					service.GetPlayers(plr, assert(args[1], "Missing target player (argument #1)"), {
						IsKicking = true,
				...
				if args[1] then
					for _2, v2 in service.GetPlayers(plr, args[1]) do
						...
						Remote.Send(
							...
							true,
							v2.Character,
							args[2] and BrickColor.new(args[2]).Color
						)
					end
				else
				...
				for ind, v in timeBans do
					...
					if timeLeft <= 0 then
						...
						table.insert(tab, {
							...
							Desc = string.format(
								"Issued by: %s | Minutes left: %d",
								v.Moderator or "%UNKNOWN%",
								minutes
							),
						})
					end
		...
		TimeMessage = {
			...
			Function = function(plr: Player, args: { string })
				...
				Functions.Message(
					"Message from " .. service.FormatPlayer(plr),
					service.BroadcastFilter(args[2], plr),
					service.GetPlayers(),
					true,
					args[1]
				)
		...
		Message = {
			...
			Function = function(plr: Player, args: { string })
				...
				Functions.Message(
					"Message from " .. service.FormatPlayer(plr),
					service.BroadcastFilter(args[1], plr),
					service.GetPlayers(),
					true
				)
			end,
		...
		Hint = {
			...
			Function = function(plr: Player, args: { string })
				...
				Functions.Hint(
					string.format("%s: %s", service.FormatPlayer(plr), service.BroadcastFilter(args[1], plr)),
					service.GetPlayers()
				)
			end,
		},
		...
		TimeHint = {
			...
			Function = function(plr: Player, args: { string })
				...
				Functions.Hint(
					string.format("%s: %s", service.FormatPlayer(plr), service.BroadcastFilter(args[2], plr)),
					service.GetPlayers(),
					tonumber(args[1])
				)
			end,
					...
					if Admin.CheckAuthority(plr, v, "warn", false) then
						...
						Remote.MakeGui(plr, "Notification", {
							...
							OnClick = Core.Bytecode(
								"client.Remote.Send('ProcessCommand','"
									.. Settings.Prefix
									.. "warnings "
									.. v.Name
									.. "')"
					...
					if Admin.CheckAuthority(plr, v, "kick-warn", false) then
						...
						if typeof(v) == "Instance" then
							...
							Core.CrossServer(
								"RemovePlayer",
								v.Name,
								"Warning from " .. service.FormatPlayer(plr),
								reason
							)
						end
			...
			Function = function(plr: Player, args: { string })
				for _, v in service.GetPlayers(plr, args[1]) do
					...
					Remote.MakeGui(plr, "Notification", {
						...
						Time = 5,
						OnClick = Core.Bytecode(
							"client.Remote.Send('ProcessCommand','" .. Settings.Prefix .. "warnings " .. v.Name .. "')"
						),
					})
				end
			end,
		...
		ForceField = {
			...
			Function = function(plr: Player, args: { string })
				for _, v in service.GetPlayers(plr, args[1]) do
					if v.Character then
						service.New("ForceField", v.Character).Visible = if args[2]
								and args[2]:lower() == "false"
							then false
							else true
					end
				end
				...
				for _, v in service.GetPlayers(plr, args[1]) do
					Routine(function()
						...
						Admin.RunCommand(
							...
							v.Name,
							"-AFK-_" .. service.FormatPlayer(v) .. "_-AFK-"
						)
						...
						repeat
							torso.CFrame = pos
							wait()
						until not v or not v.Character or not torso or not running or not torso.Parent
					end)
				end
			end,
		},
					...
					if hum then
						...
						if Settings.CommandFeedback then
							Functions.Notification(
								"God mode",
								"Character God mode has been enabled. You will not take damage from non-explosive weapons.",
								{ v },
								15,
								"Info"
							)
				...
				for _, v in service.GetPlayers(plr, args[1]) do
					...
					if hum then
						...
						if Settings.CommandFeedback then
							Functions.Notification(
								"God Mode",
								"Character god mode has been disabled.",
								{ v },
								15,
								"Info"
							)
				...
				for _, v in service.GetPlayers(plr, args[1]) do
					...
					if hum then
						...
						if Settings.CommandFeedback then
							Functions.Notification(
								"God Mode",
								"Character god mode has been enabled. You will not take any damage.",
								{ v },
								15,
								"Info"
							)
					...
					if not p then -- System event(s)
						...
						if cmd == "SendMessage" then
							...
							if filtered ~= message then
								...
								local gotIcon, status = service.Players:GetUserThumbnailAsync(
									p.UserId,
							...
							if peer then
								for pr in newSession.Users do
									if peer.UserId and peer.UserId == pr.UserId then
										...
										systemMessage(
											string.format("<i>%s removed %s from the session</i>", p.Name, pr.Name)
										)
									end
								end
							end
			...
			ListUpdater = function(plr: Player, target: Player)
				...
				if equippedTool then
					table.insert(tab, {
						...
						Desc = string.format(
							"Class: %s | %s",
							equippedTool.ClassName,
							if equippedTool:IsA("Tool")
								then "ToolTip: " .. equippedTool.ToolTip
								else "BinType: " .. equippedTool.BinType
						),
					})
				end
				...
				if backpack then
					for _, t in backpack:GetChildren() do
						table.insert(tab, {
							Text = t.Name,
							Desc = if t:IsA("BackpackItem")
								then string.format(
									"Class: %s | %s",
									t.ClassName,
									if t:IsA("Tool") then "ToolTip: " .. t.ToolTip else "BinType: " .. t.BinType
								)
				...
				for _, v in service.GetPlayers(plr, args[1]) do
					Routine(function()
						Remote.MakeGui(plr, "List", {
							...
							AutoUpdate = if args[2] and (args[2]:lower() == "true" or args[2]:lower() == "yes")
								then 1
								else nil,
							Update = "ShowBackpack",
							UpdateArg = v,
							...
							TitleButtons = {
								{
									Text = "",
									OnClick = Core.Bytecode(
										"client.Remote.Send('ProcessCommand','" .. Settings.Prefix .. "tools')"
									),
									Children = {
										{
											Class = "ImageLabel",
		...
		PlayerList = {
			...
			ListUpdater = function(plr: Player)
						...
						if type(v) == "string" and v == "NoPlayer" then
							...
							if v and service.Players:FindFirstChild(v.Name) then
								...
								table.insert(tab, {
									...
									Desc = string.format(
										"Lower: %s | Health: %d | MaxHealth: %d | WalkSpeed: %d | JumpPower: %d | Humanoid Name: %s",
										v.Name:lower(),
				...
				for i = 0.1, 5, 0.1 do
					if Functions.CountTable(tab) - 2 >= Functions.CountTable(players) then
						break
					end
					wait(0.1)
				end
			...
			Function = function(plr: Player, args: { string })
				...
				Remote.MakeGui(plr, "List", {
					...
					Size = { 300, 240 },
					AutoUpdate = if not args[1] or (args[1]:lower() == "true" or args[1]:lower() == "yes")
						then 1
						else nil,
					Update = "PlayerList",
				})
		...
		DeleteWaypoint = {
			...
			Function = function(plr: Player, args: { string })
				for i, v in Variables.Waypoints do
					if
						string.sub(string.lower(i), 1, #args[1]) == string.lower(args[1])
						or string.lower(args[1]) == "all"
					then
		...
		MakeCamera = {
			...
			Function = function(plr: Player, args: { string })
				local head = plr.Character
					and (plr.Character:FindFirstChild("Head") or plr.Character:FindFirstChild("HumanoidRootPart"))
			...
			Function = function(plr: Player, args: { string })
				...
				for _, viewer in service.GetPlayers(plr, args[1]) do
					for _, target in targets do
						...
						Functions.Hint(
							service.FormatPlayer(viewer) .. " is now viewing " .. service.FormatPlayer(target),
							{ plr }
						)
					end
				end
			...
			Function = function(plr: Player, args: { string })
				for _, v in service.GetPlayers(plr, args[1]) do
					if v.Character and v.Character.PrimaryPart then
						...
						Functions.Hint(
							service.FormatPlayer(v) .. " doesn't have a character and/or HumanoidRootPart",
							{ plr }
						)
					end
		...
		Repeat = {
			...
			Function = function(plr: Player, args: { string })
				...
				if
					string.lower(string.sub(command, 1, #Settings.Prefix + string.len("repeat")))
						== string.lower(Settings.Prefix .. "repeat")
					or string.sub(command, 1, #Settings.Prefix + string.len("loop")) == string.lower(
						Settings.Prefix .. "loop"
					)
					or string.find(command, "^" .. Settings.Prefix .. "loop")
					or string.find(command, "^" .. Settings.Prefix .. "repeat")
				then
				...
				Functions.Hint(
					"Running " .. command .. " " .. amount .. " times every " .. timer .. " seconds.",
					{ plr }
				)
				for i = 1, amount do
					if not Variables.CommandLoops[name .. command] then
						break
					end
				...
				elseif args[2] then
					if Variables.CommandLoops[name .. args[2]] then
						...
						Remote.MakeGui(
							plr,
							"Output",
							{ Title = "Output", Message = "No loops relating to your search" }
						)
					end
				else
					for i, v in Variables.CommandLoops do
				...
				if target then
					for _, v in Functions.GetPlayers(plr, target) do
						...
						for _, t in cTasks do
							table.insert(temp, {
								Text = tostring(t.Name or t.Function)
									.. "- Status: "
									.. t.Status
									.. " - Elapsed: "
									.. t.CurrentTime - t.Created,
					...
					for _, v in tasks do
						table.insert(temp, {
							Text = tostring(v.Name or v.Function)
								.. " - Status: "
								.. v.Status
								.. " - Elapsed: "
								.. (os.time() - v.Created),
					...
					if level > 0 then
						table.insert(unsorted, {
							Text = string.format(
								RANK_TEXT_FORMAT,
								service.FormatPlayer(v),
								(rankName or ("Level: " .. level))
							),
							Desc = string.format(
								RANK_DESCRIPTION_FORMAT,
								rankName or (level >= 1000 and "Place Owner") or "Unknown",
								level
							),
							SortLevel = level,
						})
		...
		Vote = {
			...
			Function = function(plr: Player, args: { string })
				...
				if not question then
					error("You forgot to supply a question!")
				end
		...
		ToolList = {
			...
			Args = {},
			Description = "Shows you a list of tools that can be obtained via the "
				.. Settings.Prefix
				.. "give command, and other useful utilities",
			AdminLevel = "Moderators",
			ListUpdater = function(plr: Player)
				local data = {
					Tools = {},
				...
				for _, tool in
					if Settings.RecursiveTools
						then Settings.Storage:GetDescendants()
						else Settings.Storage:GetChildren()
				do
		...
		InsertList = {
			...
			Function = function(plr: Player, args: { string })
				...
				for _, v in Variables.InsertList do
					table.insert(tab, v)
				end
				for _, v in HTTP.Trello.InsertList do
					table.insert(tab, v)
				end
				for i, v in tab do
		...
		ClearGUIs = {
			...
			Description = "Removes Adonis on-screen GUIs for the target player(s); if <delete all> is false, wil, only clear "
				.. Settings.Prefix
				.. "m, "
				.. Settings.Prefix
				.. "n, "
			...
			Function = function(plr: Player, args: { string })
				...
				for i, p in service.GetPlayers(plr, args[1]) do
					...
					if Settings.CommandFeedback then
						Functions.Notification(
							"Noclip",
							"Character noclip has been enabled. You will now be able to walk though walls.",
							{ p },
							15,
							"Info"
						) -- Functions.Notification(title,message,player,time,icon)
					end
				end
			end,
		},
				...
				for i, p in service.GetPlayers(plr, args[1]) do
					...
					if old then
						...
						if Settings.CommandFeedback then
							Functions.Notification(
								"Noclip",
								"Character noclip has been disabled. You will no longer be able to walk though walls.",
								{ p },
								15,
								"Info"
							) -- Functions.Notification(title,message,player,time,icon)
		...
		Jail = {
			...
			Function = function(plr: Player, args: { string })
				...
				for _, v in service.GetPlayers(plr, args[1]) do
					local cHumanoidRootPart = v.Character and v.Character.PrimaryPart
						or v.Character and v.Character:FindFirstChild("HumanoidRootPart")
			...
			Function = function(plr: Player, args: { string })
				...
				for _, v: Player in service.GetPlayers(plr, args[1]) do
					...
					if not (rootPart and head) then
						Functions.Hint(
							service.FormatPlayer(v) .. " doesn't currently have a HumanoidRootPart/Head",
							{ plr }
						)
						continue
					end
				...
				for _, v in service.GetPlayers(plr, args[1]) do
					...
					Routine(function()
						...
						for _ = 1, num do
							...
							if anim then
								animate = hum.RigType == Enum.HumanoidRigType.R15 and Deps.Assets.R15Animate:Clone()
									or Deps.Assets.R6Animate:Clone()
		...
		CopyCharacter = {
			...
			Function = function(plr: Player, args: { string })
				...
				local target_humandescrip = target
					and target.Character:FindFirstChildOfClass("Humanoid")
					and target.Character:FindFirstChildOfClass("Humanoid"):FindFirstChildOfClass("HumanoidDescription")

				assert(
					target_humandescrip,
					"Target player doesn't have a HumanoidDescription or has a locked HumanoidDescription [Cannot copy target's character]"
				)
				...
				for _, v in service.GetPlayers(plr, args[1]) do
					Routine(function()
						if
							(v and v.Character and v.Character:FindFirstChildOfClass("Humanoid"))
							and (target and target.Character and target.Character:FindFirstChildOfClass("Humanoid"))
						then
							...
							pcall(function()
								v.Character
									...
									:ApplyDescription(cl, Enum.AssetTypeVerification.Always)
							end)
				...
				for i, p in service.GetPlayers(plr, args[1]) do
					task.defer(function()
						...
						if oChar then
							...
							oPrimary = oChar.PrimaryPart
								or (oHumanoid and oHumanoid.RootPart)
								or oChar:FindFirstChild("HumanoidRootPart")
						...
						if oPos then
							...
							if newChar then
								...
								newPrimary = newChar.PrimaryPart
									or (newHumanoid and newHumanoid.RootPart)
									or oChar:FindFirstChild("HumanoidRootPart")
						...
						if humanoid then
											...
											if not found then
												assert(
													...
													"'"
														.. v
														.. "'"
														.. " is not a valid input. Run command with no arguments to see possible inputs."
												)
											end
										end
									end
							...
							if typeof(partInput) == "table" then
									...
									if not hash[v] then
										...
										assert(
											...
											"Duplicate '"
												.. v
												.. "'"
												.. " found in input. Specify each input once only."
										)
									end
								end
								...
								if rigType == Enum.HumanoidRigType.R15 then
									for i = #partInput, 1, -1 do
										...
										elseif partInput[i] == "RightLeg" then
											...
											for i = #partInput, 1, -1 do
												if
													partInput[i] == "RightUpperLeg"
													or partInput[i] == "RightLowerLeg"
								...
								if rigType == Enum.HumanoidRigType.R6 then
									for i = #partInput, 1, -1 do
										if
											partInput[i] == "RightUpperArm"
											or partInput[i] == "RightLowerArm"
											or partInput[i] == "RightHand"
										then
											table.remove(partInput, i)
										elseif
											partInput[i] == "LeftUpperArm"
											or partInput[i] == "LeftLowerArm"
											or partInput[i] == "LeftHand"
										then
											table.remove(partInput, i)
										elseif
											partInput[i] == "RightUpperLeg"
										...
										then
											table.remove(partInput, i)
										elseif
											partInput[i] == "LeftUpperLeg"
											or partInput[i] == "LeftLowerLeg"
											or partInput[i] == "LeftFoot"
										then
											table.remove(partInput, i)
		...
		PlayerColor = {
			...
			Function = function(plr: Player, args: { string })
				...
				local BodyColorProperties =
					{ "HeadColor", "LeftArmColor", "RightArmColor", "RightLegColor", "LeftLegColor", "TorsoColor" }
				...
				do
					if tool:IsA("BackpackItem") then
						if
							...
							or string.sub(string.lower(tool.Name), 1, #args[2]) == string.lower(args[2])
						then
					...
					if StarterGear then
						for _, tool in StarterGear:GetChildren() do
							if tool:IsA("BackpackItem") then
								if
									string.lower(args[2]) == "all"
									or string.find(string.lower(tool.Name), string.lower(args[2])) == 1
								then
									tool:Destroy()
								end
							end
		...
		RemoveGuis = {
			...
			Function = function(plr: Player, args: { string })
				for _, v in service.GetPlayers(plr, args[1]) do
					Remote.LoadCode(
						v,
						[[for i, v in ipairs(service.PlayerGui:GetChildren()) do if not client.Core.GetGui(v) then pcall(v.Destroy, v) end end]]
					)
				end
			end,
		},
			...
			Function = function(plr: Player, args: { string })
				for _, v in service.GetPlayers(plr, args[1]) do
					if v.Character then
						for _, tool in v.Character:GetChildren() do
							if
								tool:IsA("BackpackItem")
								and string.sub(tool.Name:lower(), 1, #args[2]) == args[2]:lower()
							then
								local hum = v.Character:FindFirstChildOfClass("Humanoid")
								if hum then
									hum:UnequipTools()
					...
					for a, tm in service.Teams:GetChildren() do
						if string.sub(string.lower(tm.Name), 1, #args[2]) == string.lower(args[2]) then
							...
							if Settings.CommandFeedback then
								Functions.Notification(
									"Team",
									"You are now on the '" .. tm.Name .. "' team.",
									{ v },
									15,
									"Info"
								) -- Functions.Notification(title,message,player,time,icon)
			...
			Function = function(plr: Player, args: { string })
				for _, player in Functions.GetPlayers(plr, args[1]) do
					...
					if Settings.CommandFeedback then
						Functions.Notification(
							"Team",
							"Your team has been reset and you are now on the Neutral team.",
							{ player },
							15,
							"Info"
						) -- Functions.Notification(title,message,player,time,icon)
		...
		TeamList = {
			...
			Function = function(plr: Player, args: { [number]: string })
				Remote.MakeGui(plr, "Teams", {
					...
					CmdSpecialPrefix = Settings.SpecialPrefix,
		...
		SetFOV = {
			...
			Function = function(plr: Player, args: { string })
				...
				for i, v in service.GetPlayers(plr, args[1]) do
					Remote.LoadCode(
						v,
						[[workspace.CurrentCamera.FieldOfView=]] .. math.clamp(tonumber(args[2]), 1, 120)
					)
				end
			end,
		},
		...
		Place = {
			...
			Function = function(plr: Player, args: { string })
				...
				local placeId = assert(
					if reservedServerInfo then reservedServerInfo.ID else tonumber(args[2]),
					"Invalid place ID or server name (argument #2)"
				)
				local teleportOptions = if reservedServerInfo
					then service.New("TeleportOptions", {
					...
					Routine(function()
						if
							Remote.MakeGuiGet(v, "Notification", {
								Title = "Teleport",
								Text = if reservedServerInfo
									then string.format("Click to teleport to server %s.", args[2])
									else string.format("Click to teleport to place %d.", placeId),
								Time = 30,
		...
		Teleport = {
			...
			Function = function(plr: Player, args: { string })
				...
				local function teleportPlayers(destination: Vector3)
					for _, v in service.GetPlayers(plr, args[1]) do
						local rootPart = v.Character
							and (v.Character.PrimaryPart or v.Character:FindFirstChild("HumanoidRootPart"))
		...
		To = {
			...
			Function = function(plr: Player, args: { string })
				Commands.Teleport.Function(
					plr,
					{ "@" .. plr.Name, assert(args[1], "Missing destination (argument #1)") }
				)
			end,
		},
				...
				for _, v in service.GetPlayers(plr, args[1]) do
					...
					if leaderstats then
						...
						if absoluteMatch and (absoluteMatch:IsA("IntValue") or absoluteMatch:IsA("NumberValue")) then
							...
							for _, st in leaderstats:GetChildren() do
								if
									(st:IsA("IntValue") or st:IsA("NumberValue"))
									and string.match(st.Name:lower(), "^" .. statName:lower())
								then
									st.Value += valueToAdd
								end
							end
		...
		SubtractFromStat = {
			...
			Function = function(plr: Player, args: { string })
				local statName = assert(args[2], "Missing stat name (argument #2)")
				local valueToSubtract =
					assert(tonumber(args[3]), "Missing/invalid numerical value to subtract (argument #3)")
				for _, v in service.GetPlayers(plr, args[1]) do
					local leaderstats = v:FindFirstChild("leaderstats")
					if leaderstats then
						local absoluteMatch = leaderstats:FindFirstChild(statName)
		...
		CustomTShirt = {
			...
			Function = function(plr: Player, args: { [number]: string })
				...
				local TShirt = ((AssetIdType == 11 or AssetIdType == 2) and service.Insert(ClothingId))
					or (AssetIdType == 1 and Functions.CreateClothingFromImageId("ShirtGraphic", ClothingId))
					or error("Item ID passed has invalid item type")
						...
						if humanoid then
							...
							if humanoidAppliedDesc then
								...
								if bCreateNewDefaultClothing then
									...
									v.Character.Humanoid:ApplyDescription(
										humDescClone,
		...
		CustomShirt = {
			...
			Function = function(plr: Player, args: { string })
				local ClothingId = tonumber(args[2])
				local AssetIdType = service.MarketPlace:GetProductInfo(ClothingId).AssetTypeId
				local Shirt = AssetIdType == 11 and service.Insert(ClothingId)
					or AssetIdType == 1 and Functions.CreateClothingFromImageId("Shirt", ClothingId)
					or error("Item ID passed has invalid item type")
		...
		CustomPants = {
			...
			Function = function(plr: Player, args: { string })
				local ClothingId = tonumber(args[2])
				local AssetIdType = service.MarketPlace:GetProductInfo(ClothingId).AssetTypeId
				local Pants = AssetIdType == 12 and service.Insert(ClothingId)
					or AssetIdType == 1 and Functions.CreateClothingFromImageId("Pants", ClothingId)
					or error("Item ID passed has invalid item type")
		...
		AvatarItem = {
			...
			Commands = {
				...
				"rightleg",
				"head",
			}, -- Legacy aliases from old commands
			Args = { "player", "ID" },
			...
			Function = function(plr: Player, args: { [number]: string })
				...
				local success, productInfo =
					pcall(service.MarketplaceService.GetProductInfo, service.MarketplaceService, itemId)
				...
				for _, v: Player in service.GetPlayers(plr, args[1]) do
					...
					if humanoid then
						...
						elseif AccessoryAssetIds[typeId] then
							if string.find(humanoidDesc[AccessoryAssetIds[typeId]], tostring(itemId)) then
								continue
							end
				...
				for i, v in workspace:GetChildren() do
					if v.Name == "ADONIS_SOUND" then
						if v.IsPaused == false then
							v:Pause()
							Functions.Hint(
								"Music is now paused | Run " .. Settings.Prefix .. "resume to resume playback",
								{ plr }
							)
						else
							Functions.Hint(
								"Music is already paused | Run " .. Settings.Prefix .. "resume to resume",
								{ plr }
							)
						end
					end
				...
				for _, v in service.GetPlayers(plr, args[1]) do
					Routine(function()
						...
						then
							...
							service.Debris:AddItem(frc, 0.1)
							service.Debris:AddItem(Attachment, 0.1)
						end
					end)
				end
			end,
		...
		TestFilter = {
			...
			Function = function(plr: Player, args: { string })
				...
				if service.RunService:IsStudio() then
					table.insert(temp, {
						Text = "!! The string has not been filtered !!",
						Desc = "Text filtering does not work in studio",
					})
				end
				for _, v in service.GetPlayers(plr, args[1]) do
					table.insert(
						temp,
						{ Text = "-- " .. v.DisplayName .. " --", Desc = v.UserId .. " (" .. v.Name .. ")" }
					)
					table.insert(temp, {
						Text = "ChatForUser: "
							.. service.TextService:FilterStringAsync(args[2], v.UserId):GetChatForUserAsync(v.UserId),
					})
					table.insert(temp, {
						Text = "NonChatForBroadcast: " .. service.TextService
							:FilterStringAsync(args[2], v.UserId)
							:GetNonChatStringForBroadcastAsync(),
					})
					table.insert(temp, {
						Text = "NonChatForUser: " .. service.TextService
							:FilterStringAsync(args[2], v.UserId)
			...
			Function = function(plr: Player, args: { string })
				...
				if assetHD == false then
					Remote.MakeGui(
						plr,
						"Output",
						{ Title = "Output", Message = "Package " .. id .. " is not supported." }
					)
					return
				end
					...
					if suc then
						for _, item in ers.Items do
							if item.Type == "UserOutfit" then
								local _, Outfit = pcall(function()
									return service.Players:GetHumanoidDescriptionFromOutfitId(item.Id)
								end)
		...
		Char = {
			...
			Function = function(plr: Player, args: { string })
				...
				local target = tonumber(string.match(args[2], "^userid%-(%d*)"))
					or assert(Functions.GetUserIdFromNameAsync(args[2]), "Unable to fetch user.")
				if target then
					local success, desc =
						pcall(service.Players.GetHumanoidDescriptionFromUserId, service.Players, target)
			...
			Function = function(plr: Player, args: { string })
				for _, v in service.GetPlayers(plr, args[1]) do
					Routine(function()
						...
						if Humanoid then
							local success, desc =
								pcall(service.Players.GetHumanoidDescriptionFromUserId, service.Players, v.UserId)
		...
		ServerLog = {
			...
			ListUpdater = function(plr: Player)
				...
				local logHistory: { { message: string, messageType: Enum.MessageType, timestamp: number } } =
					service.LogService:GetLogHistory()
		...
		LocalLog = {
			...
			NoFilter = true,
			ListUpdater = function(plr: Player, target: Player)
				return if target and target.Parent
					then Remote.Get(target, "ClientLog")
					else { "Player is currently unreachable" }
			end,
			Function = function(plr: Player, args: { string })
				for _, v in service.GetPlayers(plr, args[1]) do
					Remote.MakeGui(plr, "List", {
						...
						Update = "LocalLog",
						UpdateArg = v,
						AutoUpdate = if args[1] and (args[1]:lower() == "true" or args[1]:lower() == "yes")
							then 1
							else nil,
						Stacking = true,
						Sanitize = true,
		...
		Reverb = {
			...
			Function = function(plr: Player, args: { string }, data: {})
				...
				if not rev or not ReverbType[rev] then
					Functions.Hint(
						"Reverb type was not specified or is invalid. Opening list of valid reverb types",
						{ plr }
					)
				...
				if args[2] then
					for _, v in service.GetPlayers(plr, args[2]) do
						Remote.LoadCode(
							v,
							'game:GetService("SoundService").AmbientReverb = Enum.ReverbType[' .. rev .. "]"
						)
					end
			...
			Function = function(plr: Player, args: { string })
				...
				for _, v in service.GetPlayers(plr, args[1]) do
					Remote.Send(
						...
						"SetCore",
						"ResetButtonCallback",
						if args[2] == "true" then true else false
					)
				end
			end,
		},
		...
		ServerPerfStats = {
			...
			ListUpdater = function(plr: Player)
				local perfStats = {
					...
					{
						"HeartbeatTimeMs",
						"The total amount of time in ms it takes long it takes to update all Task Scheduler jobs",
					},
					{ "InstanceCount", "How many Instances are currently in memory" },
					{
						"MovingPrimitivesCount",
						"How many physically simulated components are currently moving in the game world",
					},
					...
					{
						"PhysicsStepTimeMs",
						"How long it takes for the physics engine to update its current state, in milliseconds",
					},
			...
			Function = function(plr: Player, args: { [number]: string })
				...
				Remote.MakeGui(plr, "List", {
					...
					AutoUpdate = if not args[1]
							or args[1]:lower() == "true"
							or args[1]:lower() == "yes"
						then 1
						else nil,
		...
		SelectPlayers = {
			...
			Description = "Shows you a list and count of players selected in the supplied argument, ex: '"
				.. Settings.Prefix
				.. "select %raiders true' to monitor people in the 'raiders' team",
			AdminLevel = "Moderators",
